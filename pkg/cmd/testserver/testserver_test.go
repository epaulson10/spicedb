...
// This is a test to showcase the deadlock that the SpiceDB team introduced into the rudd
// library at https://github.com/dalzilio/rudd/pull/1, and how it can cause a deadlock in SpiceDB
// when calling WriteSchema.
//
// The issue is that that PR introduced recursive read locking, which sync.RWMutex does not support.
// Two RLocks() are (recursively) called during how WriteSchema uses this library. A garbage collection
// finalizer calls Lock() on the same lock. The deadlock occurs if a GC occurs in
// between the two RLock() calls, as the second RLock() call will be blocked, and so will the Lock().
func TestShowcaseDeadlockIntroducedInRuddLibrary(t *testing.T) {
	...
	config := ConfigWithOptions(
		...
		WithReadOnlyGRPCServer(util.GRPCServerConfig{
			Network: "tcp",
	...
	conn, err := srv.GRPCDialContext(ctx, grpc.WithTransportCredentials(insecure.NewCredentials()))
	require.NoError(t, err)
	...
	// Absolutely hammer the garbage collector to increase the likelihood of hitting this deadlock.
	done := false
	defer func() {
		done = true
	}()
...
// I picked this schema because it reliably reproduces the issue. I think it is better suited to
// reproduce due to its size.
const deadlockTestSchema = `
definition user {}
